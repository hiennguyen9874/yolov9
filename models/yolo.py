...
from utils.general import check_yaml, LOGGER, make_divisible, print_args
...
class TripleDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        dbox3 = (
            dist2bbox(self.dfl3(box3), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        )
        y = [
...
class TripleDDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c6, c7 = make_divisible(max((ch[self.nl * 2] // 4, self.reg_max * 4, 16)), 4), max(
            (ch[self.nl * 2], min((self.nc * 2, 128)))
        )  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(
        ...
        self.cv6 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c6, 3), Conv(c6, c6, 3, g=4), nn.Conv2d(c6, 4 * self.reg_max, 1, groups=4)
            )
            for x in ch[self.nl * 2 : self.nl * 3]
        )
        self.cv7 = nn.ModuleList(
...
class Segment(Detect):
    ...
    def forward(self, x):
        ...
        if self.training:
            return x, mc, p
        return (
            (torch.cat([x, mc], 1), p) if self.export else (torch.cat([x[0], mc], 1), (x[1], mc, p))
        )
...
class DualDSegment(DualDDetect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=(), inplace=True):
        ...
        self.cv6 = nn.ModuleList(
            nn.Sequential(Conv(x, c6, 3), Conv(c6, c6, 3), nn.Conv2d(c6, self.nm, 1))
            for x in ch[: self.nl]
        )
        self.cv7 = nn.ModuleList(
            nn.Sequential(Conv(x, c7, 3), Conv(c7, c7, 3), nn.Conv2d(c7, self.nm, 1))
            for x in ch[self.nl : self.nl * 2]
        )
    ...
    def forward(self, x):
        ...
        mc = [
            torch.cat([self.cv6[i](x[i]).view(bs, self.nm, -1) for i in range(self.nl)], 2),
            torch.cat(
                [self.cv7[i](x[self.nl + i]).view(bs, self.nm, -1) for i in range(self.nl)], 2
            ),
        ]  # mask coefficients
    ...
    def __init__(
        ...
        if isinstance(m, (Detect, DDetect, Segment, DSegment, Panoptic)):
            ...
            forward = (
                ...
                if isinstance(m, (Segment, DSegment, Panoptic))
                else self.forward(x)
            )
        ...
        if isinstance(m, (DualDetect, TripleDetect, DualDDetect, TripleDDetect, DualDSegment)):
            ...
            m.inplace = self.inplace
            forward = (
                lambda x: self.forward(x)[0][0]
                if isinstance(m, (DualDSegment))
