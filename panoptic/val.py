...
from utils.metrics import box_iou, ConfusionMatrix
...
from utils.panoptic.metrics import ap_per_class_box_and_mask, Metrics, Semantic_Metrics
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes, masks, semasks) in enumerate(pbar):
        ...
        for si, (pred, proto, psemask) in enumerate(zip(preds, protos, psemasks)):
            ...
            if npr == 0:
                ...
                if save_txt:
                    save_one_txt(
                        predn, save_conf, shape, file=save_dir / "labels" / f"{path.stem}.txt"
                    )
                if save_json:
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(metrics.ap_class_index):
            LOGGER.info(
                pf % (names[c], seen, nt[c], *metrics.class_result(i), *semantic_metrics.results())
            )
...
def parse_opt():
    ...
    parser.add_argument(
        "--data", type=str, default=ROOT / "data/coco128-pan.yaml", help="dataset.yaml path"
    )
    parser.add_argument(
        "--weights", nargs="+", type=str, default=ROOT / "yolo-pan.pt", help="model path(s)"
    )
