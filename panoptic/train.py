...
from utils.panoptic.metrics import fitness, KEYS
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    names = (
        {0: "item"} if single_cls and len(data_dict["names"]) != 1 else data_dict["names"]
    )  # class names
                ...
                if plots:
                    if ni < 10:
                        plot_images_and_masks(
                            imgs, targets, masks, semasks, paths, save_dir / f"train_batch{ni}.jpg"
                        )
                    if ni == 10:
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--data", type=str, default=ROOT / "data/coco128-seg.yaml", help="dataset.yaml path"
    )
    parser.add_argument(
        "--hyp",
        type=str,
    ...
    parser.add_argument(
        "--mask-ratio", type=int, default=4, help="Downsample the truth masks to saving memory"
    )
    parser.add_argument(
        "--no-overlap", action="store_true", help="Overlap masks train faster at slightly less mAP"
    )
