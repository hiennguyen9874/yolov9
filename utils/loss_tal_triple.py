...
class ComputeLoss:
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        loss[1] = (
            0.25 * self.BCEcls(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum
        )  # BCE
        loss[1] += (
            0.25 * self.BCEcls(pred_scores2, target_scores2.to(dtype)).sum() / target_scores_sum2
        )  # BCE
        loss[1] += (
            self.BCEcls(pred_scores3, target_scores3.to(dtype)).sum() / target_scores_sum3
        )  # BCE
