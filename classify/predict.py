...
def run(
    ...
    if webcam:
        ...
        dataset = LoadStreams(
            source, img_size=imgsz, transforms=classify_transforms(imgsz[0]), vid_stride=vid_stride
        )
        bs = len(dataset)
    elif screenshot:
...
def parse_opt():
    ...
    parser.add_argument(
        "--weights", nargs="+", type=str, default=ROOT / "yolov5s-cls.pt", help="model path(s)"
    )
    parser.add_argument(
        "--source",
        type=str,
    ...
    parser.add_argument(
        "--project", default=ROOT / "runs/predict-cls", help="save results to project/name"
    )
